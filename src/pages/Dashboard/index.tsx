...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    async function loadTransactionsAndBalance(): Promise<void> {
      const { data } = await api.get<TransactionsAndBalance>('/transactions');

      const transactionsFormatted = data.transactions.map(
        (transanction: Transaction) => {
          let transactionFormated = transanction;

          transactionFormated = {
            ...transanction,
            formattedValue:
              transanction.type === 'income'
                ? formatValue(transanction.value)
                : `- ${formatValue(transanction.value)}`,
            formattedDate: formatDate(new Date(transanction.created_at)),
          };
